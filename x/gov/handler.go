...
func (h TextResolutionHandler) Deliver(ctx weave.Context, db weave.KVStore, tx weave.Tx) (*weave.DeliverResult, error) {
	...
	// key is generated by IDGenBucket
	obj := orm.NewSimpleObj(nil, resolution)
	err = h.bucket.Save(db, obj)
	if err != nil {
		return nil, errors.Wrap(err, "saving resolution")
	}
	return &weave.DeliverResult{
		// Key should be auto-set on save, we return this to the client to query by ID
		Data: obj.Key(),
	}, nil
}
