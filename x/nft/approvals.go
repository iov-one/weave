...
func (a ApprovalOptions) Validate() error {
	if a.Count == 0 || a.Count < UnlimitedCount {
		return errors.ErrInternalLegacy("Approval count should either be unlimited or above zero")
	}
	return nil
}
...
func (m Approvals) Validate(actionMaps ...map[Action]int32) error {
	for action, meta := range m {
		...
		if !isValidAction(action) {
			return errors.ErrInternalLegacy(fmt.Sprintf("illegal action: %s", action))
		}
