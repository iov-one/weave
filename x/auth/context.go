...
// can add a signer
func withSigners(ctx weave.Context, signers []weave.KeyHash) weave.Context {
	return context.WithValue(ctx, contextKeySigners, signers)
}

// GetSigners returns who signed the current Context
// may be empty
func GetSigners(ctx weave.Context) []weave.KeyHash {
	// (val, ok) form to return nil instead of panic if unset
	val, _ := ctx.Value(contextKeySigners).([]weave.KeyHash)
