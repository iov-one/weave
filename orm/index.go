...
func (i Index) Update(db weave.KVStore, prev Object, save Object) error {
	...
	switch sw {
	case s{true, true}:
		return errors.HumanErr.New("update requires at least one non-nil object")
	case s{true, false}:
	...
	return errors.HumanErr.New("you have violated the rules of boolean logic")
}
...
func (i Index) Query(db weave.ReadOnlyKVStore, mod string,
	...
	case weave.PrefixQueryMod:
		...
		return i.loadRefs(db, refs), nil
	default:
		return nil, errors.HumanErr.New("not implemented: " + mod)
	}
}
...
func (i Index) move(db weave.KVStore, prev Object, save Object) error {
	...
	if !bytes.Equal(prev.Key(), save.Key()) {
		return errors.CannotBeModifiedErr.New("cannot modify the primary key of an object")
	}
	...
	for _, newKey := range keysToAdd {
		if i.unique {
			...
			val := db.Get(k)
			if val != nil {
				return errors.DuplicateErr.New(i.name)
			}
		}
	}
...
func (i Index) remove(db weave.KVStore, index []byte, pk []byte) error {
	...
	cur := db.Get(key)
	if cur == nil {
		return errors.NotFoundErr.New("cannot remove index from nothing")
	}
	if i.unique {
		// if something else was here, don't delete
		if !bytes.Equal(cur, pk) {
			return errors.NotFoundErr.New("cannot remove index from invalid object")
		}
		db.Delete(key)
		return nil
	}
