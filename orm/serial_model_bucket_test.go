...
func TestSerialModelBucket(t *testing.T) {
	...
	key1 := []byte("c1")
	err := b.Create(db, &CounterWithID{ID: key1, Count: 1})
	assert.Nil(t, err)
...
func TestSerialModelBucketCreateWithAutoGeneratedID(t *testing.T) {
	db := store.MemStore()
	...
	// Save new without ID
	cnt = CounterWithID{ID: weavetest.SequenceID(2), Count: 444}
	err = b.Create(db, &cnt)
	assert.Nil(t, err)
...
func TestSerialModelBucketOneWrongModelType(t *testing.T) {
	...
	err := b.Create(db, &CounterWithID{ID: []byte("counter"), Count: 1})
	assert.Nil(t, err)
