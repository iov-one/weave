...
func getHTTPClient() *HTTP {
	rpcAddr := rpctest.GetConfig().RPC.ListenAddress
	return NewHTTP(rpcAddr, "/websocket")
}
...
func TestABCIQuery(t *testing.T) {
	for i, c := range GetClients() {
		...
		client.WaitForHeight(c, apph, fastWait)
...
func TestAppCalls(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		// wait before querying
		if err := client.WaitForHeight(c, apph, fastWait); err != nil {
			t.Error(err)
		}
...
func fastWait(delta int64) (abort error) {
